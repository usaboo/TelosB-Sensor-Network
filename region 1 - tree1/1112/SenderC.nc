	...
	
	{
		...
		if(len==sizeof(TimeSync))
		{
			...
			
			{
				...
				//YOU HAVE RECEIVED PROP AND PHASE DELAYS FROM NODE BELOW
	 			if(btrpkt->stage == 1 && btrpkt->nodeid_t==1111)
				{
		    		call Leds.set(3);
				...
				if(btrpkt->stage ==2 && btrpkt->nodeid_t==1113)	
				{	 
		...
		{	
			query* btrpkt = (query*)payload;
				if(btrpkt->nodeid == 1113 && btrpkt->flag!=0)
				{
					if(!busy)
					{	
				...
				if(btrpkt->nodeid == 1112 && btrpkt->flag!=0)
