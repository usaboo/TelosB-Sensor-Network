...
implementation 
{
	...
	{		
		if(len==sizeof(TimeSync))
		{
			...
			
			{
				...
				//YOU HAVE RECEIVED PROP AND PHASE DELAYS FROM NODE BELOW
	 			if(btrpkt->stage == 1 && btrpkt->nodeid_t==1100)
				{
				...
				if(btrpkt->stage ==2 && btrpkt->nodeid_t==1112)	
				{	 
		...
		if(len==sizeof(response))
		{
			...
			if((btrpkt->nodeid == 1112 || btrpkt->nodeid == 1113) && btrpkt->flag !=0)
				{
					if(!busy)
					{	
		...
		{	
			query* btrpkt = (query*)payload;
				...
				if(btrpkt->nodeid == 1111 && btrpkt->flag!=0)
				{	
					response* newpkt = (response*)(call Packet.getPayload(&pkt, sizeof (response)));
