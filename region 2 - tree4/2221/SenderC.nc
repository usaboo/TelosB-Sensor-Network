...
implementation 
{
	...
	{		
		...
		if(len==sizeof(response))
		{
			...
			if((btrpkt->nodeid == 2223 || btrpkt->nodeid == 2222) && btrpkt->flag !=0)
				{
					if(!busy)
					{	
		...
		{	
			query* btrpkt = (query*)payload;
				if((btrpkt->nodeid == 2223 || btrpkt->nodeid == 1123) && btrpkt->flag!=0)
				{
					if(!busy)
					{	
				...
				if(btrpkt->nodeid == 2221 && btrpkt->flag!=0)
				{	
					response* newpkt = (response*)(call Packet.getPayload(&pkt, sizeof (response)));
