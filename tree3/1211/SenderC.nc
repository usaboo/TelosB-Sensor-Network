	...
	{		
		if(len==sizeof(TimeSync))
		{
			...
			
			{
				...
				if(btrpkt->stage ==2 && btrpkt->nodeid_t==1212)	
				{	 
		...
		if(len==sizeof(response))
		{
			...
			if((btrpkt->nodeid == 1212 || btrpkt->nodeid == 1213) && btrpkt->flag !=0)
				{
					if(!busy)
					{	
		...
		{	
			query* btrpkt = (query*)payload;
				...
				if(btrpkt->nodeid == 1211 && btrpkt->flag!=0)
				{	
					response* newpkt = (response*)(call Packet.getPayload(&pkt, sizeof (response)));
