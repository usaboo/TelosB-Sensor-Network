	...
	
	{
		...
		if(len==sizeof(TimeSync))
		{
			...
			
			{
				...
				//YOU HAVE RECEIVED PROP AND PHASE DELAYS FROM NODE BELOW
	 			if(btrpkt->stage == 1 && btrpkt->nodeid_t==2121)
				{
		    		call Leds.set(3);
				...
				if(btrpkt->stage ==2 && btrpkt->nodeid_t==2123)	
				{	 
		...
		{	
			query* btrpkt = (query*)payload;
				...
				if(btrpkt->nodeid == 2122 && btrpkt->flag!=0)
