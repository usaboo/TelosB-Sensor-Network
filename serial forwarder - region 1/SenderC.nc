...
implementation 
{
	...
	event message_t* Receive.receive(message_t* msg, void* payload, uint8_t len)  
	{
		...
		if(len==sizeof(TimeSync))
		{
			...
			bool below = FALSE;
			//call Leds.set(7);
			if(btrpkt->nodeid_t==1111 || btrpkt->nodeid_t==1121 || btrpkt->nodeid_t==1211 || btrpkt->nodeid_t==1221)
				above =TRUE;
			if(btrpkt->nodeid_t==2100)
				below =TRUE;
			if(above || below)
			{
		...
		{	
			...
			if((btrpkt->nodeid>=1100 && btrpkt->nodeid<1224) && btrpkt->flag!=0)
			{
