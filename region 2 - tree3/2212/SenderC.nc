	...
	
	{
		...
		if(len==sizeof(TimeSync))
		{
			...
			
			{
				...
				//YOU HAVE RECEIVED PROP AND PHASE DELAYS FROM NODE BELOW
	 			if(btrpkt->stage == 1 && btrpkt->nodeid_t==2211)
				{
		    		call Leds.set(3);
				...
				if(btrpkt->stage ==2 && btrpkt->nodeid_t==2213)	
				{	 
		...
		{	
			query* btrpkt = (query*)payload;
				if(btrpkt->nodeid == 2213 && btrpkt->flag!=0)
				{
					if(!busy)
					{	
				...
				if(btrpkt->nodeid == 2212 && btrpkt->flag!=0)
